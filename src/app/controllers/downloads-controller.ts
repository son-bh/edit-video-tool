...
export function createDownloadsController(context: Pick<AppContext, 'jobStore'>) {
  return {
    downloadScript(request: Request, response: Response): void {
      ...
      downloadIfExists(job?.outputs.scriptSrt, 'script.srt', 'script.srt is not available.', response);
    },
    downloadTranscript(request: Request, response: Response): void {
      const job = getOwnedJob(context.jobStore, request);
      downloadIfExists(job?.outputs.transcriptSrt, 'script.whisper.srt', 'script.whisper.srt is not available.', response);
    },
    downloadSegments(request: Request, response: Response): void {
      const job = getOwnedJob(context.jobStore, request);
      downloadIfExists(job?.outputs.segmentZip, 'segments.zip', 'Segment archive is not available.', response);
    },
    downloadFinalVideo(request: Request, response: Response): void {
      const job = getOwnedJob(context.jobStore, request);
      downloadIfExists(job?.outputs.finalVideo, 'final-video.mp4', 'Final video is not available.', response);
    },
    ...
    downloadFinalVideoWithAudioSubtitles(request: Request, response: Response): void {
      ...
      downloadIfExists(
        ...
        'final-video-with-audio-subtitles.mp4',
