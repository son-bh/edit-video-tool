...
const { zipDirectory } = require('./archive');
const { createProgressReporter, createSubtitleLogger, createVideoLogger } = require('./progress');
const {
...
async function runSubtitleJob(payload, progress) {
  ...
  if (payload.transcriptPath) {
    ...
    require('node:fs').writeFileSync(scriptSrtPath, output, 'utf8');
